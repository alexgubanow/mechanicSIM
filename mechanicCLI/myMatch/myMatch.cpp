...
namespace myMatch
{
	public ref struct MainFuncs
	{
		...
		enum class IntegrSchems { euler, verlet, gear };
		enum class Models { liner, moonriv, particle };
		...
		/*ref class CosMatrix
		{
		public static double get_lij(double[] x, double[] xp1)
		{
		return Math.Sqrt(
		((xp1[0] - x[0]) * (xp1[0] - x[0])) +
		((xp1[1] - x[1]) * (xp1[1] - x[1])) +
		...
		public static double[] get_a1(double lij, double[] x, double[] xp1)
		{
		return new double[3]{
		(xp1[0] - x[0]) / lij,
		(xp1[1] - x[1]) / lij,
		...
		public static double[] get_a2(double lik, double[] x, double[] k)
		{
		return new double[3]{
		(k[0] - x[0]) / lik,
		(k[1] - x[1]) / lik,
		...
		public static double[] get_a3(double[] a1, double[] a2)
		{
		return new double[3]{
		a1[1] * a2[2] - a1[2] * a2[1],
		a2[0] * a1[2] - a1[0] * a2[2],
		...
		public static double[] to_loc(double[] Ux, double[] a1, double[] a2, double[] a3)
		{
		return new double[3]{
	...
	public:	
		...
		int calcMovement(String^ flnameExtLoad, array<point>^ points, Models model, IntegrSchems shema, CalcTypes CalcType, MaterialModels MaterialModeltype, Retypes Retype, bool IsConsoleOut,
